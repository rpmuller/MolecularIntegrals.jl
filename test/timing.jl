...
# vrr3 0.014459 seconds (6.29 k allocations: 289.375 KiB, 97.21% compilation time)
...
function hrr_timings()
    ...
    cx,cy,cz = dx,dy,dz = xyza
    aexpn=bexpn=cexpn=dexpn = ex
    ashell,bshell,cshell,dshell = (2,1,2,1)
    #@time MolecularIntegrals.hrr(aexpn,ax,ay,az,aI,aJ,aK, bexpn,bx,by,bz,bI,bJ,bK, cexpn,cx,cy,cz,cI,cJ,cK, dexpn,dx,dy,dz,dI,dJ,dK);
    ...
    @time MolecularIntegrals.hrr4(ashell,bshell,cshell,dshell, aexpn,bexpn,cexpn,dexpn, A,B,C,D);
end
...
# hrr4: 0.003234 seconds (44.50 k allocations: 2.051 MiB)
